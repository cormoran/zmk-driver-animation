...
/**
 * HSL chosen over HSV/HSB as it shares the same parameters with LCh or
 * HSLuv. The latter color spaces could be interesting to experiment with
 * because of their perceptual uniformity, but it would come at the cost of
 * some performance. Using the same parameters would make it easy to toggle
 * any such behavior using a single config flag.
 *
...
/**
 * Converts ZMK's RGB (float) to Zephyr's led_rgb (uint8_t) format.
 */
